...
while row_index < directors_db.length do
  ...
  while column_index < directors_db[row_index].length do
    ...
    puts "\tCoordinate [#{coord}] points to an #{directors_db[row_index][column_index].class} of length #{inner_len}"
 
    inner_index = 0
    while inner_index < inner_len do
      puts "\t\t (#{coord}, #{inner_index}) is: #{directors_db[row_index][column_index][inner_index]}"
      inner_index += 1
    end
